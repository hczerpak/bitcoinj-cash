...
package cash.bitcoinj.tools;

import cash.bitcoinj.core.*;
import cash.bitcoinj.crypto.*;
import cash.bitcoinj.store.*;
...
import cash.bitcoinj.protocols.payments.PaymentProtocol;
import cash.bitcoinj.protocols.payments.PaymentProtocolException;
...
import cash.bitcoinj.uri.BitcoinURIParseException;
import cash.bitcoinj.utils.BriefLogFormatter;
import cash.bitcoinj.wallet.Wallet.BalanceType;
...
import static cash.bitcoinj.core.Coin.parseCoin;
...
public class WalletTool {
    ...
    private static cash.bitcoin.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("bitcoin")) {
            ...
            try {
                paymentRequest = cash.bitcoin.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
